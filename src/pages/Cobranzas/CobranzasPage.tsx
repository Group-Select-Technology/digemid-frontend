...
const PAGE_SIZE = 10;
const COBRANZAS_FOLDER = 'cobranzas';
...
export default function CobranzasPage() {
  ...
  const fetchAssets = useCallback(async (pageIndex: number) => {
    setLoading(true);
    setError(null);
    try {
      const response = await assetsService.getAll({
        ...
        folder: COBRANZAS_FOLDER,
      });
      setAssets(response.data);
      setMeta(response.meta);
    } catch {
      setError('Error al cargar las imágenes de cobranzas.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const result = await assetsService.getOne(term);
  ...
  const openDetail = async (asset: Asset) => {
    ...
    try {
      const result = await assetsService.getOne(asset.reference);
  ...
  const columns: Column<Asset>[] = [
    ...
    },
    {
      ...
      render: (a) => (
        <span className="text-sm text-gray-600 dark:text-gray-400">{a.folder}</span>
      ),
    },
    {
      header: 'Tamaño',
  ...
  const emptyMessage =
    searchResults !== null
      ...
      : 'No hay imágenes registradas en la carpeta de cobranzas.';
  ...
  return (
    <>
      <PageMeta title="Cobranzas" description="Imágenes de vouchers de cobranzas" />
      <PageBreadCrumb pageTitle="Cobranzas" />
      ...
      <Modal
        isOpen={detailOpen}
        onClose={closeDetail}
        className="max-w-4xl overflow-y-auto p-4 sm:p-6"
      >
